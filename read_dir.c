...
static void read_dir(int, const struct ext2_inode*, const struct ext2_group_desc*, char*);
...
static void read_dir(int fd, const struct ext2_inode *inode, const struct ext2_group_desc *group, char* nomeArquivo)
{
	void *block;
	...
	if (S_ISDIR(inode->i_mode)) {
		...
		while((size < inode->i_size) && entry->inode) {
			...
			// if(strcpm(nomeArquivo == entry->name)){
			// 	return entry->inode;
			// }
